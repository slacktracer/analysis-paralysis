...
vec3 cube_1_position = vec3(-4, 0.5, -4);

Surface sdScene(vec3 p) {
    vec3 floorColor = vec3(1. + 0.7*mod(floor(p.x) + floor(p.z), 2.0));
    Surface co = sdFloor(p, floorColor);
    co = minWithColor(co, sdBox(p, vec3(1), cube_1_position, vec3(1, 0, 0), identity())); // left cube
...
void main() {
    ...
    vec3 col = vec3(0);
    vec3 lp = cube_1_position; // lookat point (aka camera target)
//    vec3 lp = vec3(0, 0.5, -4); // lookat point (aka camera target)
    vec3 ro = vec3(0, 5, 0); // ray origin that represents camera position

    float cameraRadius = 10.;
    ro.x = cameraRadius * cos(time) + lp.x; // convert to polar
    ro.z = cameraRadius * sin(time) + lp.z;
